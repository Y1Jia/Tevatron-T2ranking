...
class EncoderModel(nn.Module):
    ...
    def forward(self, query: Dict[str, Tensor] = None, passage: Dict[str, Tensor] = None):
        ...
        if self.training:
            ...
            # not use in-batch negatives 
            # batch_size = q_reps.size(0)
            # hidden_size = q_reps.size(-1)
            # p_reps = p_reps.view(batch_size, -1, hidden_size)   # shape: (batch_size, train_n_passages, hidden_size)
            # scores = torch.matmul(q_reps.unsqueeze(1), p_reps.transpose(1, 2)).squeeze(1)   # shape: (batch_size, train_n_passages)

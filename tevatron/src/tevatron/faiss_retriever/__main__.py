...
def search_queries(retriever, q_reps, p_lookup, args):
    ...
    psg_indices = [[str(p_lookup[x]) for x in q_dd] for q_dd in all_indices]    # 查表p_look_up，转化成pid
...
def write_ranking(corpus_indices, corpus_scores, q_lookup, ranking_save_file):
    with open(ranking_save_file, 'w') as f:
        for qid, q_doc_scores, q_doc_indices in zip(q_lookup, corpus_scores, corpus_indices):
            ...
            score_list = sorted(score_list, key=lambda x: x[0], reverse=True)   # 按照分数降序排列
...
def main():
    ...
    parser.add_argument('--quiet', action='store_true') # disable tqdm

    args = parser.parse_args()

    index_files = glob.glob(args.passage_reps)  # list of passages index files
    logger.info(f'Patt0ern match found {len(index_files)} files; loading them into index.')
    ...
    if args.save_text:
        write_ranking(psg_indices, all_scores, q_lookup, args.save_ranking_to)  # qid pid scores 每个query对应的部分按照分数降序排列
    else:
